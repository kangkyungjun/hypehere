...
- [ ] **환경변수 설정 확인** (⚠️ 중요!)
  ...
  - [ ] **환경변수 변경 시 프론트엔드 재시작 필수** (`./restart-dev.sh` 실행)
- [ ] 커밋 메시지 작성
- [ ] 중요 변경사항이면 팀에 공지
...
### 문제 이력
**2025-10-19 (Issue 1)**: 로컬 환경변수가 AWS 프로덕션 환경에 배포되어 로그인 401 에러 발생
**2025-10-19 (Issue 2)**: 환경변수 변경 후 프론트엔드 재시작 없이 실행하여 로그인 401 에러 및 관심사 목록 표시 안됨
...
### ⚠️ 중요: 환경변수 변경 시 프론트엔드 재시작 필수

**문제**: Next.js는 환경변수를 시작 시점에만 로드합니다. `.env.local` 파일을 변경해도 실행 중인 개발 서버는 이전 값을 계속 사용합니다.
...
1. 모든 Node 프로세스 종료
2. 환경변수 설정 확인 및 출력
3. 새로운 환경변수로 개발 서버 재시작

**언제 재시작이 필요한가**:
- ✅ `.env.local` 파일 수정 후
- ✅ Git pull로 환경변수 변경사항 받은 후
