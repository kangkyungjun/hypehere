...
class TickerHeaderWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasBarChart = currentPrice != null && currentPrice > 0 &&
      ...
      child: Column(
        ...
        children: [
          ...
          if (tickerInfo?.name != null)
            Text(
              (Localizations.localeOf(context).languageCode == 'ko' && tickerInfo!.nameKo != null)
  ...
  Widget _buildMiniBarChart(BuildContext context, double? price60, double? price30, double currentPrice) {
    return SizedBox(
      width: 100,
      height: 120,
      child: CustomPaint(
        ...
        painter: _MiniBarChartPainter(
          ...
          baselineColor: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.15),
  ...
  double? _findPriceNDaysAgo(int days) {
    ...
    if (closest != null && minDiff <= 7 && closest.close != null && closest.close! > 0) {
...
class _MiniBarChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // diffPct 계산
    final diff60 = price60 != null ? (price60! - currentPrice) / currentPrice * 100 : null;
    final diff30 = price30 != null ? (price30! - currentPrice) / currentPrice * 100 : null;
    ...
    final maxAbsDiff = maxDiff.isEmpty ? 1.0 : maxDiff.reduce((a, b) => a > b ? a : b);

    // 기준선 Y 동적 계산
    // 위로 가는 바가 있으면 위쪽 공간 필요, 아래로 가면 아래쪽 공간 필요
    final hasUp = (diff60 != null && diff60 > 0) || (diff30 != null && diff30 > 0);
    final hasDown = (diff60 != null && diff60 < 0) || (diff30 != null && diff30 < 0);
    ...
    // 바 그리기 헬퍼
    void drawBar(double x, double diffPct, double pastPrice, String periodLabel) {
      final isUp = diffPct > 0; // 과거 > 현재 → 위로 (빨강)
      final color = isUp ? redColor : greenColor;

      // 바 높이: 비례 축소, 최소 4px
      final availableHeight = isUp ? (baselineY - textBlockHeight) : (size.height - baselineY - textBlockHeight);
      final effectiveMaxHeight = availableHeight.clamp(4.0, maxBarHeight);
      final barHeight = (effectiveMaxHeight * (diffPct.abs() / maxAbsDiff)).clamp(4.0, effectiveMaxHeight);
      ...
      if (isUp) {
        ...
        _drawText(canvas, periodLabel, x + barWidth / 2, baselineY - barHeight - 34,
            9, sublabelColor, FontWeight.normal);
        _drawText(canvas, '\$${pastPrice.toStringAsFixed(2)}', x + barWidth / 2, baselineY - barHeight - 23,
            8, labelColor, FontWeight.normal);
        _drawText(canvas, '+${diffPct.toStringAsFixed(1)}%', x + barWidth / 2, baselineY - barHeight - 12,
            8, redColor, FontWeight.bold);
      } else {
        // 바 아래로 (기준선 아래)
        final rect = RRect.fromRectAndCorners(
        ...
        _drawText(canvas, '${diffPct.toStringAsFixed(1)}%', x + barWidth / 2, baselineY + barHeight + 3,
            8, greenColor, FontWeight.bold);
        _drawText(canvas, '\$${pastPrice.toStringAsFixed(2)}', x + barWidth / 2, baselineY + barHeight + 14,
            8, labelColor, FontWeight.normal);
        _drawText(canvas, periodLabel, x + barWidth / 2, baselineY + barHeight + 25,
            9, sublabelColor, FontWeight.normal);
      }
    }
  ...
  void _drawText(Canvas canvas, String text, double centerX, double topY,
    ...
    textPainter.paint(
      canvas,
      Offset(centerX - textPainter.width / 2, topY),
    );
  }
