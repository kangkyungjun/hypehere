...
```bash
# 1. 파일 전송
scp -i ~/Downloads/hypehere-key.pem <local-file> ubuntu@43.201.45.60:/tmp/
...
```bash
scp -i ~/Downloads/hypehere-key.pem backend/accounts/views.py ubuntu@43.201.45.60:/tmp/views.py
